...
#script to graph the stability of protein B during simulations using the 4gvb protien (fig s17)
...
plot.data <- data.frame(x=c(
  ...
  plot.data.un5$x),
  lab_place=c(
    rep(1970,length(Un1$an.binding)),
    rep(200,length(Un2$an.binding)),
...
survival.lines <- function(df) {
  ...
  g <- g + xlab('Time')
  g<-  g + coord_cartesian(ylim=c(-46.5, -43.5),xlim=c(0, 2200))
  ...
  save_plot("STAB_B_with_controls_other_prot_nice.pdf",g)
  return(g)
}
