...
sites_and_time_data_C<-function(file,sumC,sites_timeC,BindBoth){
  ...
  for(i in seq(1,2000,50)){
    ...
    for(j in 1:length(split_C)){
      ...
      sticky_vector<-sticky_by_site(split_C[j]) 
...
sites_and_time_data_A<-function(file,sumA,sites_timeA,BindBoth){
  ...
  for(i in seq(1,2000,50)){
    ...
    for(j in 1:length(split_A)){
      ...
      sticky_vector<-sticky_by_site(split_A[j]) 
...
site_timeA_all_time<-t(read.csv(file="C:/Users/User/Documents/dup_alignments/diffs_A.csv"))
site_timeB_all_time<-t(read.csv(file="C:/Users/User/Documents/dup_alignments/diffs_B.csv"))
site_timeC_all_time<-t(read.csv(file="C:/Users/User/Documents/dup_alignments/diffs_C.csv"))
...
C_wt<-sites_and_time_data_C("C:/Users/User/Documents/dup_alignments/M_SIM1_csv_files/M_SIM1_", sumC,site_timeC_all_time,TRUE)

A_wt<-sites_and_time_data_A("C:/Users/User/Documents/dup_alignments/M_SIM1_csv_files/M_SIM1_", sumA,site_timeA_all_time,TRUE)

#make dataframes from the bind B and not B' data
C_nobind<-sites_and_time_data_C("C:/Users/User/Documents/dup_alignments/M_SIM2_csv_files/M_SIM2_",sumC, site_timeC_all_time,FALSE)

A_nobind<-sites_and_time_data_A("C:/Users/User/Documents/dup_alignments/M_SIM2_csv_files/M_SIM2_", sumA,site_timeA_all_time,FALSE)

#combine both the wt and nobind data into one dataframe
Sticky_C<-rbind.data.frame(C_wt,C_nobind)  
Sticky_A<-rbind.data.frame(A_wt,A_nobind)  



#cut out everything then doesnt matter, ie. all the data where we never observed a difference
dfA<-Sticky_A[, colSums(Sticky_A != 0) > 0]

#just do it by 100s's rather then 50s, this can be changed to get higher or lower grandulatity out of the figure
dfA<-dfA[which(dfA$Gen %% 100 == 0) ,]
...
dfA$Gen <- factor(dfA$Gen, levels = levels)
...
justleg<-ggplot(dfA, aes(x = dfA$Sticky.20, y = dfA$Gen, fill=paste(dfA$Exp))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, alpha=.5, bandwidth=band,scale=scaler_var, from=min(Sticky), to=max(Sticky))+ylab("Substitutions")+xlab(lab)

A20<-ggplot(dfA, aes(x = dfA$Sticky.20, y = dfA$Gen, fill=paste(dfA$Exp,dfA$Sel.20))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, alpha=.5, bandwidth=band, scale=scaler_var, from=min(Sticky), to=max(Sticky))+ylab("Substitutions")+xlab(lab)
...
A20<-A20+dis+breaks+dis2
A20<-A20+theme_tA


C9<-ggplot(df, aes(x = df$Sticky.9, y = df$Gen, fill=paste(df$Exp,df$Sel.9))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, alpha=.5,bandwidth=band, scale=scaler_var,from=min(Sticky), to=max(Sticky))
C9<-C9+theme_t+dis+breaks+xlab(lab)+dis2


C44<-ggplot(df, aes(x = df$Sticky.44, y = df$Gen, fill=paste(df$Exp,df$Sel.44))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, alpha=.5, bandwidth=band,scale=scaler_var,from=min(Sticky), to=max(Sticky))

C44<-C44+theme_t+dis+breaks+xlab(lab)+dis2

C46<-ggplot(df, aes(x = df$Sticky.46, y = df$Gen, fill=paste(df$Exp,df$Sel.46))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, alpha=.5, bandwidth=band,scale=scaler_var,from=min(Sticky), to=max(Sticky))

C46<-C46+theme_t+dis+breaks+xlab(lab)+dis2

C47<-ggplot(df, aes(x = df$Sticky.47, y = df$Gen, fill=paste(df$Exp,df$Sel.47))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, alpha=.5, bandwidth=band,scale=scaler_var,from=min(Sticky), to=max(Sticky))
C47<-C47+theme_t+dis+breaks+xlab(lab)+dis2

C68<-ggplot(df, aes(x = df$Sticky.68, y = df$Gen, fill=paste(df$Exp,df$Sel.68))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, alpha=.5, bandwidth=band,scale=scaler_var,from=min(Sticky), to=max(Sticky))
C68<-C68+theme_t+dis+breaks+xlab(lab)+dis2

legend<-get_legend(justleg)

#put all the individual plots together
pp<- plot_grid( A20+ theme(legend.position="none"),
                C9,
                C44,
                C46,
                C47,
...
ggplot2::ggsave("C:/Users/User/Documents/dup_alignments/Sticky_All_grid_nice.pdf",plot=p, width = 11, height = 9,units = "in")