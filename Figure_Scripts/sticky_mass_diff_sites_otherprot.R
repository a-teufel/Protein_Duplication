...
sites_and_time_data_C<-function(file,sumC,sites_timeC,BindBoth){
  ...
  for(i in seq(1,2000,50)){
    ...
    sequences = read_csv( name_EXP, col_names = c("simulation", "proteinA", "proteinB1", "proteinB2", "proteinC1","proteinC2","empty"))
    proteinC<-paste(sequences$proteinC1,sequences$proteinC2,sep="")
    sequences$proteinC=proteinC
    ...
    colnames(sum_sticky)<-paste(seq(1,74))
    colnames(which_time)<-paste(seq(1,74))
    temp_C_sticky<-NULL
    ...
    if(BindBoth){
      ...
      temp_C_sticky<-data.frame(Gen=rep(i-1,nrow(sum_sticky)),  Sticky=sum_sticky, Exp=rep("Bind B and Not B'",nrow(sum_sticky)),Sel=which_time)  
    }
...
sites_and_time_data_B<-function(file,sumB,sites_timeB,BindBoth){
  ...
  for(i in seq(1,2000,50)){
    ...
    #read in data
    sequences = read_csv( name_EXP, col_names = c("simulation", "proteinA", "proteinB1", "proteinB2", "proteinC1","proteinC2","empty"))
    proteinB<-paste(sequences$proteinB1,sequences$proteinB2,sep="")
    sequences$proteinB=proteinB
    ...
    if(BindBoth){
      ...
      temp_B_sticky<-data.frame(Gen=rep(i-1,nrow(sum_sticky)),  Sticky=sum_sticky, Exp=rep("Bind B and Not B'",nrow(sum_sticky)),Sel=which_time)  
    }
...
sites_and_time_data_A<-function(file,sumA,sites_timeA,BindBoth){
  ...
  for(i in seq(1,2000,50)){
    ...
    #dataframe is organized by sites on the columns.
    colnames(sum_sticky)<-paste(seq(1,77))
    colnames(which_time)<-paste(seq(1,77))
    ...
    if(BindBoth){
      ...
      temp_A_sticky<-data.frame(Gen=rep(i-1,nrow(sum_sticky)),  Sticky=sum_sticky, Exp=rep("Bind B and Not B'",nrow(sum_sticky)),Sel=which_time)
...
site_timeA_all_time<-t(read.csv(file="C:/Users/User/Documents/dup_alignments/diffs_A_other.csv"))
site_timeB_all_time<-t(read.csv(file="C:/Users/User/Documents/dup_alignments/diffs_B_other.csv"))
site_timeC_all_time<-t(read.csv(file="C:/Users/User/Documents/dup_alignments/diffs_C_other.csv"))
...
C_wt<-sites_and_time_data_C("C:/Users/User/Documents/dup_alignments/M_SIM1_4gvb_csv_files/M_SIM1_4gvb_", sumC,site_timeC_all_time,TRUE)

B_wt<-sites_and_time_data_B("C:/Users/User/Documents/dup_alignments/M_SIM1_4gvb_csv_files/M_SIM1_4gvb_", sumB,site_timeB_all_time,TRUE)

#make dataframes from the bind B and not B' data
C_nobind<-sites_and_time_data_C("C:/Users/User/Documents/dup_alignments/M_SIM2_4gvb_csv_files/M_SIM2_4gvb_",sumC, site_timeC_all_time,FALSE)

B_nobind<-sites_and_time_data_B("C:/Users/User/Documents/dup_alignments/M_SIM2_4gvb_csv_files/M_SIM2_4gvb_", sumB,site_timeB_all_time,FALSE)
...
justleg<-ggplot(dfB, aes(x = dfB$Sticky.21, y = dfB$Gen, fill=paste(dfB$Exp))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE,alpha=.5, bandwidth=band,scale=scaler_var,from=min(Sticky), to=max(Sticky))+ylab("Substitutions")+xlab(lab)

B21<-ggplot(dfB, aes(x = dfB$Sticky.21, y = dfB$Gen, fill=paste(dfB$Exp,dfB$Sel.21))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, alpha=.5,bandwidth=band, scale=scaler_var,from=min(Sticky), to=max(Sticky))+ylab("Substitutions")+xlab(lab)
...
#dis for sticky
#dis2<-scale_x_continuous(breaks=c(-1,0,1),expand = c(0.01, 0))
#figure out which ones to gray out
breaks<-scale_fill_cyclical(breaks = c("Bind Both 0","Bind Both 1", "Bind B and Not B' 0","Bind B and Not B' 1"),
                            values = c(  "#B2D4FF","#0072B2" ,"#FFDDD5","#D55E00"))

breaks_leg<-scale_fill_cyclical(labels=c("bind both  ", "bind B and not B'"),breaks = c("Bind Both","Bind B and Not B'"),values = c("#0072B2","#D55E00"),name = "Simulation",guide = "legend")

#make a plot just for its legend
justleg<-justleg+breaks_leg+dis2 +guides(fill=guide_legend(nrow=1,byrow=TRUE))+ theme(legend.title.align=0.5) 



B21<-B21+dis+breaks+dis2
B21<-B21+theme_tA


B24<-ggplot(dfB, aes(x = dfB$Sticky.24, y = dfB$Gen, fill=paste(dfB$Exp,dfB$Sel.24))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE,bandwidth=band, alpha=.5, scale=scaler_var,from=min(Sticky), to=max(Sticky))
B24<-B24+theme_t+dis+breaks+xlab(lab)+dis2


C21<-ggplot(df, aes(x = df$Sticky.21, y = df$Gen, fill=paste(df$Exp,df$Sel.21))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE,bandwidth=band, alpha=.5, scale=scaler_var,from=min(Sticky), to=max(Sticky))
C21<-C21+theme_t+dis+breaks+xlab(lab)+dis2

C24<-ggplot(df, aes(x = df$Sticky.24, y = df$Gen, fill=paste(df$Exp,df$Sel.24))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE,bandwidth=band, alpha=.5, scale=scaler_var,from=min(Sticky), to=max(Sticky))
C24<-C24+theme_t+dis+breaks+xlab(lab)+dis2

C59<-ggplot(df, aes(x = df$Sticky.59, y = df$Gen, fill=paste(df$Exp,df$Sel.59))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, bandwidth=band,alpha=.5, scale=scaler_var,from=min(Sticky), to=max(Sticky))
C59<-C59+theme_t+dis+breaks+xlab(lab)+dis2

C60<-ggplot(df, aes(x = df$Sticky.60, y = df$Gen, fill=paste(df$Exp,df$Sel.60))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE, bandwidth=band,alpha=.5, scale=scaler_var,from=min(Sticky), to=max(Sticky))
C60<-C60+theme_t+dis+breaks+xlab(lab)+dis2
...
C63<-ggplot(df, aes(x = df$Sticky.63, y = df$Gen, fill=paste(df$Exp,df$Sel.63))) + geom_density_ridges(rel_min_height = .001,color = "white",panel_scaling=FALSE,bandwidth=band, alpha=.5,scale=scaler_var,from=min(Sticky), to=max(Sticky))

C63<-C63+theme_t+dis+breaks+xlab(lab)+dis2
...
pp<- plot_grid( B21+ theme(legend.position="none"),
                B24,
                C21,
                C24,
                C59,
...
p<-plot_grid(pp,legend, rel_heights = c(3, .25),nrow=2)

ggplot2::ggsave("C:/Users/User/Documents/dup_alignments/Mass_All_grid_other_nice.pdf",plot=p, width = 11, height = 9,units = "in")