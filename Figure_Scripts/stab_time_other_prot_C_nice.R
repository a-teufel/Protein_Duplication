...
#script to graph the stability of protein B' during simulations using the 4gvb protien (fig s6)
...
plot.data <- data.frame(x=c(
  ...
  lab_place=c(
    ...
    rep(1970,length(Un2$an.binding)),
    rep(300,length(Un5$an.binding))),
  y=c(
    plot.data.un1$y,
    plot.data.un2$y,
    plot.data.un5$y),
...
survival.lines <- function(df) {
  ...
  g <- g + xlab('Time')
  g<-  g + coord_cartesian(ylim=c(-49, -25),xlim=c(0, 2200))
  ...
  save_plot("STAB_C_with_controls_other_prot_nice.pdf",g)
  return(g)
}
...
#pdf(paste('/home/ateufel/Marcott_Sim/STAB_AB.pdf', sep=''), width=10, height=10,onefile=FALSE)

#s<-plot_grid(plotlist=p,ncol=1)
#s
#dev.off()
