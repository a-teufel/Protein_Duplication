...
rm(list = ls())
#script to graph the stability of protein A' during simulations when A is the duplicate (fig s5)
...
plot.data <- data.frame(x=c(
  ...
  plot.data.un5$x),
  lab_place=c(
    rep(1950,length(Un1$an.binding)),
    rep(1800,length(Un2$an.binding)),
    rep(1950,length(Un5$an.binding))),
  y=c(
    plot.data.un1$y,
    plot.data.un2$y,
    plot.data.un5$y),
...
survival.lines <- function(df) {
  ...
  g <- g + ylab(expression(paste(Delta,"G of A' ")))
  g <- g + xlab('Time')
  g<-  g + coord_cartesian(ylim=c(-140, -123),xlim=c(0, 2200))
  ...
  g<-g+geom_text_repel(
    ...
    segment.size = 0.35,  force = 15,nudge_y=c(-1,3,1),
  ...
  save_plot("STAB_C_with_controls_dup_nice.pdf",g)
  return(g)
}
