...
name_WT<-paste("C:/Users/User/Documents/dup_alignments/M_SIM1_csv_files_first_500/M_SIM1_1.csv",sep="")  
...
name_EXP<-paste("C:/Users/User/Documents/dup_alignments/M_SIM2_csv_files_first_500/M_SIM2_1.csv",sep="")  
...
for(i in seq(11,250,10)){
  ...
  name_WT<-paste("C:/Users/User/Documents/dup_alignments/M_SIM1_csv_files_first_500/M_SIM1_",i,".csv",sep="")  
  ...
  b<-table( factor( diffsitesB, levels = 1:73)) 
...
for(i in seq(11,250,10)){       
  ...
  name_EXP<-paste("C:/Users/User/Documents/dup_alignments/M_SIM2_csv_files_first_500/M_SIM2_",i,".csv",sep="")  
  nobind_sequences = read_csv(name_EXP, col_names = c("simulation", "proteinA", "proteinB", "proteinC", "empty"))
...
#graph A
A<-ggplot(bothA, aes(Inter,color=Simulation,fill=Simulation))+ geom_density(alpha=.5)+scale_fill_manual(values=cols)+ scale_colour_manual(values=cols)+ylab("Density")+xlab("Probability of Interface Substitution")+ scale_y_continuous(expand = c(0.001, 0))+ scale_x_continuous(limit=c(0,.09))
...
B<-ggplot(bothB, aes(Inter,color=Simulation,fill=Simulation))+ geom_density(alpha=.5)+scale_fill_manual(values=cols)+ 
  scale_colour_manual(values=cols)+xlab("Probability of Interface Substitution")+ylab("Density")+scale_y_continuous(expand = c(0.001, 0),limit=c(0,10),breaks=c(0,2,4,6,8,10))+ scale_x_continuous(limit=c(0,.45))
...
ggplot2::ggsave("C:/Users/User/Documents/dup_alignments/Interface_prob_expand_updated.pdf",plot=p, width = 11, height = 9,units = "in")
