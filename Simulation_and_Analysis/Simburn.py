...
# run this program like: python Simburn.py mypdb
...
# This program is used to for the burnin phase
...
def main():
    ...
    # Initialize Rosetta.
    init(extra_options='-mute basic -mute core -mute protocol')
    ...
    data = ['Variant,ChainA,ChainB,InterfaceAB,"delta-delta-G",Probability,Generation\n']
    ...
    inter_AB=InterfaceEnergy_split_init(initial_poseAB)
    ...
    data.append('WT,' + str(wt_a)+','+str(wt_b)+','+str(inter_AB)+',0.0,0.0,0\n')

    init_score=score_all(initial_pose,sf,min_mover,beta,threshold_a, threshold_b,threshold_inter_ab,score_type)
    ...
    while i < max_accept_mut:
	    ...
	    mutant_pose=mutate_residue_chain(mutant_pose, mut_location, proposed_res, PACK_RADIUS, sf)
            ...
            mut_score=score_all(mutant_pose,sf,min_mover,beta,threshold_a, threshold_b,threshold_inter_ab,score_type)
		...
		if i>=0:
		   #save name and energy change
	    	   data.append(variant_name +',' + str(mut_score['a'])+','+str(mut_score['b'])+','+str(mut_score['ab'])+',' + str(mut_score['score'] - init_score['score']) + "," + str(probability) + "," + str(gen) + "\n")
...
def InterfaceEnergy_split_init(pdb):
...
def restrict_non_nbrs_from_repacking_chain(pose, res, task, pack_radius):
    ...
    for i in range(1, pose.total_residue() + 1):
        # only pack the mutating residue and any within the pack_radius and only repack on the same chain that is being mutated
            ...
            #if i != res and dist > pack_radius or not pose.pdb_info().chain(i) == pose.pdb_info().chain(res) :
            if dist > pack_radius:
